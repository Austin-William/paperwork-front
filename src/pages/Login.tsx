...
function LoginPage() {
    ...
    const [password, setPassword] = useState(mockData.user_table[0].password);
    ...
    async function handleSubmit(event: any) {
        ...
        // })
        cookies.set('loginToken', { loginToken: mockData.user_table[0].token, email: email }, {
            path: '/',
            secure: true,
            sameSite: 'none'
        });
                        ...
                        <div>
                            ...
                            <Modal className='Login-modal' style={{ content: { background: "rgba(45,45,45,1)" } }} overlayClassName='process-idea-modal-cancel-overlay' isOpen={isOpenModal} onRequestClose={onCloseModal}>
                                ...
                                <div className='Login-box-email'>
                                    ...
                                    <button className={buttonDisabled ? 'Login-send-mail-button disabled' : 'Login-send-mail-button'} onClick={handleForgotPassword} aria-label='button-send-mail-forgot-password'
                                        //disabled={buttonDisabled}
                                        disabled
                                    >
                                        <FiSend className='Login-title-icons' size={30} />
                                    </button>
                                </div>
                            </Modal>
                        </div>
